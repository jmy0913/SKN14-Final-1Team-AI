...
OUTPUT_DIR = "firebase_firestore_fulltext6"
MAX_PAGES = 800
CRAWL_DELAY_SEC = 1
HEADLESS = True  # 창 띄우려면 False

# ▶ 패널(탭) 내부의 설명/표도 수집할지 여부
...
TAB_LABEL_TOKENS = {
    ...
    "c#","dotnet",".net","net","maven","gradle","c++","unity","web"
}
...
def _filter_code_candidates(arr):
    ...
    for s in _join_unique(arr):
        ...
        if len(s.strip()) < 6 and not re.search(r"[;{}=<>()\[\]]", s):
            continue
        out.append(s)
    return out
...
def _normalize_lang_label(label: str) -> str:
    lab = (label or "").strip()
    lab = re.sub(r"\s*\((async|beta|preview|head)\)\s*$", "", lab, flags=re.I)
    lab = re.sub(r"\s*\(.*?\)\s*$", "", lab)
    low = lab.lower()
    if "web" in low and "modular" in low: return "Web (modular API)"
    if "web" in low and ("namespaced" in low or "namespace" in low): return "Web (namespaced API)"
    lab = lab.replace("자바", "Java").replace("파이썬", "Python").replace("자바스크립트", "JavaScript")
    return lab

def _tab_label(el, default_name):
    try:
        txt = (el.text or "").strip()
        if txt: return _normalize_lang_label(txt)
        for k in ("aria-label","data-lang","data-code-lang","data-language","title"):
            v = el.get_attribute(k)
            if v: return _normalize_lang_label(v.strip())
    except Exception:
        pass
    return default_name
...
return out;
        ...
        pass
    return _join_unique([_normalize_lang_label(x) for x in labels])
...
def _activate_overflow_item(tablist, label):
    try:
        ...
        norm = _normalize_lang_label(label).strip().lower()
        item = None
        for it in menu.find_elements(By.CSS_SELECTOR, "[role=menuitem], a, button, li>button, li>a"):
            try:
                ...
                lab = _normalize_lang_label(lab)
                if lab and lab.strip().lower() == norm:
                    item = it; break
            except Exception:
                continue
        if not item:
...
def _collect_snippets_for_tab(tab, tablist, lang_label):
    ...
    try:
        ...
        if INCLUDE_PANEL_TEXT_IN_TABS:
            ...
            tbls = _panel_tables(snap_root)
            if tbls:
                results.append((f"{lang_label} · 표", "### 표\n" + "\n\n".join(tbls), False))
        ...
        for block_title, node_scope in _iter_section_scopes(article):
            ...
            for idx, tablist in enumerate(tablists, start=1):
                ...
                try:
                    ...
                    for t_i in range(len(tabs)):
                        try:
                            ...
                            norm = _normalize_lang_label(lang).lower()
                            if norm in clicked: continue
                            root = _find_panel_for_tab(tab, tablist) or tablist
                            before = _panel_state(root)
                            res = _collect_snippets_for_tab(tab, tablist, lang)
                            after = _panel_state(root)
                            if res and after != before:
                                code_items.extend(res)
                                clicked.add(norm)
                                seen_signatures.add(after)
                ...
                try:
                    ...
                    if more:
                        ...
                        for lab in labels:
                            norm = _normalize_lang_label(lab).lower()
                            if norm in clicked:
                                continue
                            snap_root = tablist
                            ...
                            try:
                                ...
                                pass
                            after = _panel_state(snap_root)
                            if after in seen_signatures:
                                continue

                            codes = _visible_codes_in(snap_root)
                            if codes:
                                for i, c in enumerate(codes, 1):
                                    code_items.append((f"{_normalize_lang_label(lab)} · 셀#{i}", c, True))
                                clicked.add(norm)
                                seen_signatures.add(after)
                            ...
                            if INCLUDE_PANEL_TEXT_IN_TABS:
                                ctx = _panel_plain_texts(snap_root)
                                if ctx:
                                    code_items.append((f"{_normalize_lang_label(lab)} · 설명", "\n\n".join(ctx), False))
                                tbls = _panel_tables(snap_root)
                                if tbls:
                                    code_items.append((f"{_normalize_lang_label(lab)} · 표", "### 표\n" + "\n\n".join(tbls), False))
                except Exception:
                    pass
            ...
            if code_items:
                ...
                for (lab, txt, _is_code) in code_items:
                    key = (_normalize_lang_label(lab), _norm_text(txt))
