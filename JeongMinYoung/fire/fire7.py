...
# ========= 기본 설정 =========
OUTPUT_DIR = "firebase_firestore_fulltext7"
MAX_PAGES = 800
CRAWL_DELAY_SEC = 1
...
LANGUAGE_ALIASES = {
    ...
    ".net": ["csharp","dotnet","cs","c#"], "net": ["csharp","dotnet","cs","c#"], "c#": ["csharp","dotnet","cs","c#"],
...
# (옵션) 탭/패널 내부 설명도 같이 저장할지
INCLUDE_PANEL_TEXT_IN_TABS = False
...
def setup_driver():
    ...
    chrome_options.add_argument("--headless=new")  # 무창 실행
...
# ---- 인라인 토큰 필터(짧은/연산자 토큰 제거)
OP_ONLY_RE = re.compile(r'^[<>=!+\-*/%&|^~?:]{1,4}$')
BRACKETS_RE = re.compile(r'^[()\[\]{}]+$')
PUNCT_ONLY_RE = re.compile(r'^[,.;]{1,2}$')

def _is_trivial_inline_code(t: str) -> bool:
    if not t: return True
    t = t.strip()
    if "\n" in t: return False
    if OP_ONLY_RE.fullmatch(t) or BRACKETS_RE.fullmatch(t) or PUNCT_ONLY_RE.fullmatch(t): return True
    if t in {"->","=>","<>","===","!=="}: return True
    if re.fullmatch(r'[A-Za-z_][\w\.\-/#]*', t) and len(t) <= 24: return True
    if len(t) <= 18 and not re.search(r'\s', t): return True
    return False
...
def _guess_lang_label(text: str, default_label: str = "코드") -> str:
    ...
    if head.startswith("<") and re.search(r'</?[A-Za-z][^>]*>', t): return "HTML/XML"
...
def _find_panel_for_tab(tab, tablist):
    """다양한 연결 방식(aria-controls, aria-labelledby, data-tab, <a href=#id>)로 패널을 찾아낸다."""
    ...
    # 2) aria-labelledby (앵커 id 연결)
    try:
        a = tab.find_element(By.CSS_SELECTOR, "a[id]")
        aid = a.get_attribute("id") or ""
        if aid:
            if host:
                try: return host.find_element(By.CSS_SELECTOR, f"section[role='tabpanel'][aria-labelledby='{aid}']")
                except Exception: pass
            try: return driver.find_element(By.CSS_SELECTOR, f"section[role='tabpanel'][aria-labelledby='{aid}']")
            except Exception: pass
    except Exception:
        pass
    ...
    # 3) data-tab 키로 매칭
    try:
        key = tab.get_attribute("data-tab") or tab.get_attribute("id") or ""
        key = key.strip()
        if key:
            if host:
                try: return host.find_element(By.CSS_SELECTOR, f"section[role='tabpanel'][data-tab='{key}'], section[role='tabpanel']#{'tabpanel-'+key}")
                except Exception: pass
            try: return driver.find_element(By.CSS_SELECTOR, f"section[role='tabpanel'][data-tab='{key}'], section[role='tabpanel']#{'tabpanel-'+key}")
            except Exception: pass
    except Exception:
        pass

    return None  # 마지막 폴백은 상위에서 tablist 자체를 사용


def _panel_state(root):
    """패널 상태(코드 길이 합산) — shadow/light DOM 모두"""
    try:
        return driver.execute_script("""
const r = arguments[0];
let count=0,total=0;

const add = t => { if(t){ count++; total += t.length; } };

// clipboard
r.querySelectorAll('[data-clipboard-text]').forEach(b=>{
  const t=b.getAttribute('data-clipboard-text')||''; add(t.trim());
});

// syntax attr
r.querySelectorAll('pre[syntax],code[syntax]').forEach(n=>{
  const t=(n.innerText||n.textContent||'').trim(); add(t);
});

// devsite-code (shadow + light)
r.querySelectorAll('devsite-code,devsite-snippet').forEach(host=>{
  const sr=host.shadowRoot;
  if(sr){ sr.querySelectorAll('pre,code').forEach(n=>{ const t=(n.innerText||n.textContent||'').trim(); add(t); }); }
  host.querySelectorAll('pre,code').forEach(n=>{ const t=(n.innerText||n.textContent||'').trim(); add(t); });
});

// 일반 pre (인라인 code 제외)
r.querySelectorAll('pre').forEach(n=>{
...
// 3) devsite-code / devsite-snippet (shadow + light DOM)
root.querySelectorAll('devsite-code,devsite-snippet').forEach(host=>{
  ...
  host.querySelectorAll('pre,code').forEach(n=>{
    if(!vis(n)) return;
    const t=(n.innerText||n.textContent||'').trim(); if(t) out.push(t);
  });
});

// 4) 일반 pre (인라인 code 제외, 표 내부 제외)
...
return out;
    ...
    codes = _filter_code_candidates(_js_get(root))
    return codes
...
def _find_more_button(tablist):
    for sel in MORE_BTN_SEL.split(","):
        ...
        try:
            ...
            if btn and btn.is_displayed(): return btn
        except Exception:
            continue
    return None
...
const lab = el => (el.innerText||el.textContent||el.getAttribute('aria-label')||el.getAttribute('data-code-lang')||'').trim();
menu.scrollTop=0;
for(let i=0;i<80;i++){
  menu.querySelectorAll('[role=menuitem],a,button,li>button,li>a').forEach(it=>{
    const t=lab(it); if(t && !seen.has(t)){ seen.add(t); out.push(t); }
  });
...
def _activate_overflow_item(tablist, label):
    try:
        ...
        item = None
        for it in menu.find_elements(By.CSS_SELECTOR, "[role=menuitem], a, button, li>button, li>a"):
            try:
                lab = (it.text or "").strip() or (it.get_attribute("aria-label") or "").strip() \
                      or (it.get_attribute("data-code-lang") or "").strip()
                if lab and lab.strip().lower() == label.strip().lower(): item = it; break
            except Exception:
                continue
        if not item:
            driver.execute_script("document.body.click();")
...
return out;
    ...
    for t in _join_unique(texts):
        if _looks_like_tab_labels(t): continue
        cleaned.append(t)
    return cleaned
...
def _collect_snippets_for_tab(tab, tablist, lang_label):
    ...
    try:
        driver.execute_script("arguments[0].scrollIntoView({block:'center'});", snap_root)
    except Exception:
        pass
    try:
...
def _iter_section_scopes(article):
    ...
    first = heads[0]
    if first > 0: yield "서문", children[0:first]
...
def _extract_plain_texts(nodes):
    ...
    for n in nodes:
        try:
            ...
            if tag in TAGS:
                if driver.execute_script("return !!arguments[0].closest('table')", n): pass
                else:
...
root.querySelectorAll('p,li,dt,dd,blockquote').forEach(el=>{
  ...
  if (t) out.push(t);
    ...
    for t in _join_unique(para):
        if "도움이 되었나요" in t or "google developer" in t.lower(): continue
        if _looks_like_tab_labels(t): continue
        cleaned.append(t)
    return cleaned
...
root.querySelectorAll('table').forEach(t=>{
  ...
  if (ths.length) ths.forEach(th=>headers.push((th.innerText||th.textContent||'').trim()));
    ...
    if(firstTr) firstTr.querySelectorAll('th').forEach(th=>headers.push((th.innerText||th.textContent||'').trim()));
  }
  const rows = [];
  const trs = t.querySelectorAll('tbody tr, tr');
  trs.forEach(tr=>{
    const cells=[]; tr.querySelectorAll('th,td').forEach(td=>{
      let x=(td.innerText||td.textContent||'').trim(); cells.push(x);
    });
...
root.querySelectorAll('devsite-code,devsite-snippet').forEach(host=>{
  ...
  host.querySelectorAll('pre').forEach(n=>{ if(!vis(n)) return; const t=(n.innerText||n.textContent||'').trim(); if(t) out.push(t); });
});
...
return out;
            ...
            for i, txt in enumerate(_filter_code_candidates(blocks), 1):
                label = _guess_lang_label(txt, "코드")
                items.append((f"{label} · 셀#{i}", _norm_text(txt), True))
        except Exception:
            continue
    return items
...
return out;
            ...
            for i, c in enumerate(_filter_code_candidates(codes), 1):
                ...
                else: label = _guess_lang_label(c, "코드")
...
def collect_page_text(article) -> str:
    ...
    try:
        for block_title, node_scope in _iter_section_scopes(article):
            ...
            if plain: section_parts.append("\n\n".join(plain))

            # (1.5) 표
            tables_md = _extract_tables(node_scope)
            if tables_md: section_parts.append("### 표\n" + "\n\n".join(tables_md))
                ...
                try:
                    ...
                    for t_i in range(len(tabs)):
                        try:
                            ...
                            if res: code_items.extend(res); clicked.add(lang)
                ...
                try:
                    ...
                    if more:
                        ...
                        for lab in labels:
                            ...
                            snap_root = _find_panel_for_tab(tablist, tablist) or tablist
                            before = _panel_state(snap_root)
                            ok = _activate_overflow_item(tablist, lab)
                            if not ok: continue
                            try:
                            ...
                            if codes:
                                for i, c in enumerate(codes, 1):
                                    code_items.append((f"{lab} · 셀#{i}", c, True))
                                clicked.add(lab)
                            if INCLUDE_PANEL_TEXT_IN_TABS:
                                ctx = _panel_plain_texts(snap_root)
                                if ctx: code_items.append((f"{lab} · 설명", "\n\n".join(ctx), False))
                except Exception:
                    pass
