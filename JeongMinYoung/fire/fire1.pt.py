...
OUTPUT_DIR = "firebase_firestore_fulltext2"
MAX_PAGES = 800
CRAWL_DELAY_SEC = 1
...
TAB_LABEL_TOKENS = {
    ...
    # Firebase 문서의 드롭다운(더보기)에 자주 뜨는 라벨
    "dart","flutter","c++","async","비동기","more","더보기","unity"
}
...
def url_to_safe_filename(url: str) -> str:
    """도메인별로 안전한 파일명 생성 (firebase + cloud 혼용 지원)"""
    try:
        p = urlparse(url)
        base = (p.netloc + (p.path or "/")).rstrip("/")
        fname = re.sub(r'[/\\?%*:|"<>]', "_", base).strip("_") or "index"
        if p.query:
...
const out = [];
document.querySelectorAll('article a[href], main a[href], nav a[href], aside a[href]').forEach(a=>{
...
root.querySelectorAll('pre,code').forEach(n=>{
  if(n.closest('table')) return;  // 표 내부 제외
...
# ========= 드롭다운(더보기)까지 모두 긁기 =========
def _collect_overflow_menu_codes(tablist):
    results = []
    snap_root = _closest_selector_host(tablist) or tablist

    try:
        triggers = tablist.find_elements(
            By.CSS_SELECTOR,
            "button[aria-haspopup], .devsite-tabs__more, .devsite-tabs__dropdown, .devsite-tabs__toggle, .devsite-tabs-overflow-button"
        )
        triggers = [t for t in triggers if (t.text or t.get_attribute("aria-label") or "").strip()]
    except Exception:
        triggers = []

    if not triggers:
        try:
            triggers = driver.execute_script("""
...
return cand;
    ...
    def _visible_menu_items(trigger):
        return driver.execute_script("""
const vis=el=>{const s=getComputedStyle(el); if(s.display==='none'||s.visibility==='hidden'||+s.opacity===0) return false;
               const b=el.getBoundingClientRect(); return b.width>0&&b.height>0;};
const menus = Array.from(document.querySelectorAll(
  "[role='menu'], [role='listbox'], .devsite-tabs-overflow-menu, .devsite-tabs__menu, .devsite-dropdown-menu"
)).filter(vis);

let items=[];
for(const m of menus){
  items = items.concat(Array.from(m.querySelectorAll(
    "*[role='menuitem'], *[role='option'], tab[role='menuitem'], tab, button, a, li>button, li>a"
  )).filter(vis));
}
return items;
        """, trigger) or []
    ...
    for trigger in triggers:
        ...
        try:
            ...
            time.sleep(0.1)
            items_snapshot = _visible_menu_items(trigger)
        except Exception:
            items_snapshot = []

        for idx in range(len(items_snapshot)):
            try:
                before_state = _panel_state(snap_root)
                try:
                    driver.execute_script("arguments[0].click();", trigger)
                ...
                items_now = _visible_menu_items(trigger)
                if idx >= len(items_now):
                    break
                item = items_now[idx]
                lang_label = _tab_label(item, f"More-{idx+1}")
        ...
        for block_title, node_scope in _iter_section_scopes(article):
            ...
            for idx, tablist in enumerate(tablists, start=1):
                try:
                    ...
                    # 드롭다운(더보기) 내부 항목 수집
                    overflow_items = _collect_overflow_menu_codes(tablist)
                    if overflow_items:
                        code_items.extend(overflow_items)
