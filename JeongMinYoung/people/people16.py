...
OUTPUT_DIR = "people_docs_fulltext"
MAX_PAGES = 500
...
def setup_driver():
    ...
    # 본문이 채워질 때까지의 하이드레이션을 보장하기 위해 'eager'
...
def _extract_plain_texts(nodes):
    """섹션 범위에서 일반 텍스트(p, li, 표 등)를 수집. 코드/탭/위젯 영역은 제외."""
    para = []
    SEL = ",".join([
...
const b = document.querySelector('.devsite-article-body');
if (!b) return false;
