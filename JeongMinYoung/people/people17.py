...
OUTPUT_DIR = "people_docs_fulltext_edit"
MAX_PAGES = 500
...
def _extract_plain_texts(nodes):
    """
    섹션 범위에서 일반 텍스트(p, li, 표 등)를 수집.
    ※ 루트 노드 자체가 p/li/... 인 경우도 포함(이전 버그 fix)
    """
    para = []
    TAGS = {"P","LI","DT","DD","BLOCKQUOTE","TABLE","THEAD","TBODY","TFOOT","TR","TH","TD"}
...
const out = [];
root.querySelectorAll('p,li,dt,dd,blockquote,table,thead,tbody,tfoot,tr,th,td').forEach(el=>{
