...
OUTPUT_DIR = "people_docs_fast_with_text"
MAX_PAGES = 500
...
def setup_driver():
    ...
    drv.implicitly_wait(0)  # 명시적 대기만 사용
    return drv
...
def _visible_codes_in(root):
    """
    root(=현재 탭 패널) 내부에서 '표시 중' 코드만 추출.
    단계별로 결과가 생기면 즉시 반환하여 라벨/노이즈 혼입 방지.
    """
...
root.querySelectorAll('pre,code').forEach(n=>{
  if(!vis(n)) return;
  if(n.closest('devsite-code,devsite-snippet')) return; // shadow는 위에서 처리
  const t=(n.innerText||n.textContent||"").trim();
  if(t) out.push(t);
});
return out;
    ...
    # 1) clipboard
    codes = _filter_code_candidates(_js_visible_clip(root))
    if codes: return codes
    # 2) syntax
    codes = _filter_code_candidates(_js_visible_syntax(root))
    if codes: return codes
    # 3) shadow
    codes = _filter_code_candidates(_js_visible_shadow(root))
    if codes: return codes
    # 4) fallback
    return _filter_code_candidates(_js_visible_fallback(root))
...
def _iter_section_scopes(article):
    ...
    for idx, el in enumerate(children):
        try:
            if (el.tag_name or "").lower() in ("h2", "h3"):
                heads.append(idx)
        except Exception:
            continue
    ...
    first = heads[0]
    if first > 0:
        preface_nodes = children[0:first]
        # 실제 텍스트가 있으면 서문 출력
        if _nodes_have_visible_text(preface_nodes):
            yield "서문", preface_nodes
    ...
    for i, hidx in enumerate(heads):
        ...
        scope_nodes = children[hidx+1:end]
        yield title, scope_nodes

def _nodes_have_visible_text(nodes):
    try:
        return any(_extract_plain_texts([n]) for n in nodes)
    except Exception:
        return False
...
root.querySelectorAll(sel).forEach(el=>{
  if(!vis(el)) return;
  // 코드/탭/위젯 영역 제외
  if(el.closest('[role=\"tablist\"], [role=\"tabpanel\"], devsite-code, devsite-snippet, pre, code')) return;
