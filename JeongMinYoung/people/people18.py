...
OUTPUT_DIR = "people_docs_fulltext_edit1"
MAX_PAGES = 500
...
# ========= (고친 곳) 일반 텍스트 추출 (표는 여기서 제외) =========
def _extract_plain_texts(nodes):
    """
    섹션 범위에서 일반 텍스트(p, li, dt, dd, blockquote)를 수집.
    ※ 표(table)는 전용 파서에서 처리 (중복 방지)
    """
    para = []
...
def _extract_tables(nodes):
    """
    섹션 범위에서 table을 마크다운 표로 변환.
    """
    md_tables = []

    def format_md(headers, rows):
        # 헤더 비었으면 첫 행을 헤더로 승격
        if not headers and rows:
            headers = rows[0]
            rows = rows[1:]
        headers = [ _norm_text(h) for h in (headers or []) ]
        rows = [ [ _norm_text(c) for c in r ] for r in (rows or []) ]
...
root.querySelectorAll('table').forEach(t=>{
  ...
  const headers = [];
  // thead > th 우선, 없으면 첫 tr의 th 사용
  const ths = t.querySelectorAll('thead th');
  if (ths.length) {
    ths.forEach(th => { const x=(th.innerText||th.textContent||'').trim(); headers.push(x); });
  } else {
    const firstTr = t.querySelector('tr');
    if (firstTr) {
      firstTr.querySelectorAll('th').forEach(th => { const x=(th.innerText||th.textContent||'').trim(); headers.push(x); });
    }
  }
  const rows = [];
...
return out;
            ...
            for headers, rows in tables:
                md = format_md(headers, rows)
                if md: md_tables.append(md)
        except Exception:
            continue

    return _join_unique(md_tables)
