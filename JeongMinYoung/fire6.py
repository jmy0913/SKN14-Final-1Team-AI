...
OUTPUT_DIR = "firebase_firestore_fulltext5"
MAX_PAGES = 800
CRAWL_DELAY_SEC = 1
...
# ---- 인라인 토큰 필터 + 도식 감지 ----
INLINE_TOKEN_RE = re.compile(r'^[\w\.\-/#]+(?:\(\))?$')  # get, get(), cities 등

def _is_diagramish_code(t: str) -> bool:
    if not t:
        return True
    t = _norm_text(t)
    ...
    if not lines:
        return True
    # 표의 비교연산자(<, <=, == ...)만 있는 토막
    if len(lines) <= 2 and all(re.fullmatch(r'[<>=!]+', ln) for ln in lines):
        return True
    # 코드 키워드/기호 점수
    code_markers = sum(1 for ln in lines if re.search(
        r'(;|\{|\}|\(|\)|=>|\bimport\b|\bfrom\b|\bclass\b|\bdef\b|\bawait\b|\breturn\b|\bnew\b|\blet\b|\bconst\b|\bvar\b|\.doc\(|\.collection\(|->)',
        ln))
    # 다이어그램/키:값 나열 위주
    kv_lines = sum(1 for ln in lines if re.search(r'\w\s*:\s*["\'\w\[\]{}]', ln))
    diagram_tokens = sum(1 for ln in lines if re.search(r'\b(class|collections_bookmark)\b', ln))
    if code_markers == 0 and (kv_lines >= max(3, int(0.7 * len(lines))) or diagram_tokens >= len(lines) // 2):
        return True
    return False
...
def _is_trivial_inline_code(t: str) -> bool:
    ...
    if "\n" in t:
        return False
    if re.search(r'[;={}\[\]<>]|//|/\*|\b(import|class|def|await|return|var|let|const|final|new)\b', t):
        return False
...
def _filter_code_candidates(arr):
    ...
    for s in _join_unique(arr):
        ...
        if _is_diagramish_code(s):     # 도식/토막/표기호만 → 코드로는 제외
            continue
        out.append(s)
    return out
...
return out;""", r) or []
    ...
    pieces = []
    for getter in (_js_visible_clip, _js_visible_syntax, _js_visible_shadow):
        pieces += getter(root)
    return _filter_code_candidates(pieces)
...
menu.scrollTop = 0;
for (let i=0;i<50;i++){
  const items = Array.from(menu.querySelectorAll('[role=menuitem],a,button,li>button,li>a'));
...
# ===== 탭 헬퍼: 보이는 탭/활성 패널/라벨 활성화 =====
def _get_tabs(tablist):
    tabs = []
    try:
        tabs.extend(tablist.find_elements(By.CSS_SELECTOR, '[role="tab"]'))
        tabs.extend(tablist.find_elements(By.CSS_SELECTOR, "tab, button, a, li>button, li>a"))
        tabs = [t for t in tabs if t.is_displayed()]
    except Exception:
        pass
    return tabs

def _get_active_tab(tablist):
    tabs = _get_tabs(tablist)
    for t in tabs:
        cls = (t.get_attribute("class") or "")
        if t.get_attribute("aria-selected") == "true" or "devsite-active" in cls:
            return t
    return tabs[0] if tabs else None

def _get_active_panel_for_tablist(tablist):
    t = _get_active_tab(tablist)
    if not t:
        return tablist
    p = _find_panel_for_tab(t, tablist)
    return p or tablist

def _activate_label(tablist, label: str) -> bool:
    """label 탭이 보이면 직접 클릭, 아니면 더보기에서 꺼내 클릭."""
    label_lc = (label or "").strip().lower()

    # 보이는 탭 중에 있으면 바로 클릭
    for t in _get_tabs(tablist):
        if _tab_label(t, "").strip().lower() == label_lc:
            driver.execute_script("arguments[0].scrollIntoView({block:'center'});", t)
            driver.execute_script("arguments[0].click();", t)
            return True

    # 더보기에서 꺼내기 → 새 탭이 생기면 그 탭을 다시 클릭
    if _activate_overflow_item(tablist, label):
        try:
            WebDriverWait(driver, 4).until(
                lambda d: any(_tab_label(t, "").strip().lower() == label_lc for t in _get_tabs(tablist))
            )
        except TimeoutException:
            # 어떤 페이지는 메뉴 클릭이 곧바로 패널도 바꿈
            return True
        for t in _get_tabs(tablist):
            if _tab_label(t, "").strip().lower() == label_lc:
                driver.execute_script("arguments[0].click();", t)
...
def _collect_for_label(tablist, label):
    """label 탭을 활성화하고 해당 패널에서 코드 수집."""
    panel_before = _get_active_panel_for_tablist(tablist)
    before_state = _panel_state(panel_before)

    ok = _activate_label(tablist, label)
    if not ok:
        return []
    ...
    try:
        WebDriverWait(driver, 6).until(
            lambda d: _panel_state(_get_active_panel_for_tablist(tablist)) != before_state
        )
    except TimeoutException:
        pass

    panel = _get_active_panel_for_tablist(tablist)
    codes = _visible_codes_in(panel)
    return [(f"{label} · 셀#{i}", c, True) for i, c in enumerate(codes, 1)]
...
def _collect_diagram_texts(nodes):
    """코드 블록처럼 보이지만 실제로는 다이어그램/데이터 예시인 것들을 텍스트로 수집."""
    items = []
    for n in nodes:
        try:
            texts = driver.execute_script("""
...
const vis=el=>{const s=getComputedStyle(el); if(s.display==='none'||s.visibility==='hidden'||+s.opacity===0) return false;
               const b=el.getBoundingClientRect(); return b.width>0&&b.height>0;};
// 탭/표/코드 위젯 내부는 제외, 일반 pre/code만
root.querySelectorAll('pre,code').forEach(el=>{
  if (el.closest('table')) return;
  if (el.closest('[role="tablist"],[role="tabpanel"],devsite-code,devsite-snippet')) return;
  if (!vis(el)) return;
  const t=(el.innerText||el.textContent||'').trim();
  if (t) out.push(t);
});
...
def _collect_loose_code(nodes):
    """탭 밖의 '진짜 코드'만 수집 (클립보드/문법/섀도우)."""
    items = []
    for n in nodes:
        try:
            codes = []
            # data-clipboard-text
            clips = driver.execute_script("""
const root=arguments[0], out=[];
const vis=el=>{const s=getComputedStyle(el); if(s.display==='none'||s.visibility==='hidden'||+s.opacity===0) return false;
               const b=el.getBoundingClientRect(); return b.width>0&&b.height>0;};
root.querySelectorAll('[data-clipboard-text]').forEach(b=>{
...
return out;""", n) or []
            ...
            syns = driver.execute_script("""
const root=arguments[0], out=[];
const vis=el=>{const s=getComputedStyle(el); if(s.display==='none'||s.visibility==='hidden'||+s.opacity===0) return false;
               const b=el.getBoundingClientRect(); return b.width>0&&b.height>0;};
root.querySelectorAll('pre[syntax],code[syntax]').forEach(el=>{
  if(!vis(el)) return; const t=(el.innerText||el.textContent||'').trim(); if(t) out.push(t);
});
return out;""", n) or []
            codes += syns
            # devsite-code/shadow
...
root.querySelectorAll('devsite-code,devsite-snippet').forEach(host=>{
  ...
  const sr=host.shadowRoot; if(!sr) return;
  sr.querySelectorAll('pre,code').forEach(el=>{
    if(!vis(el)) return; const t=(el.innerText||el.textContent||'').trim(); if(t) out.push(t);
  });
});
return out;""", n) or []
...
const out=[];
document.querySelectorAll('pre[syntax],code[syntax]').forEach(n=>{
...
return out;
            ...
            if not codes:
                ...
                if _norm_text(body_txt):
                    # iframe 본문이 전부 텍스트일 때만 보조로 담는다 (필터에서 걸러짐)
                    codes = [body_txt]
...
def collect_page_text(article) -> str:
    ...
    try:
        for block_title, node_scope in _iter_section_scopes(article):
            ...
            # (1.7) 도식/ASCII 블록 (코드가 아닌 텍스트로 수집)
            diagrams = _collect_diagram_texts(node_scope)
            if diagrams:
                section_parts.append("### 도식/예시\n" + "\n\n".join(diagrams))

            # (2) 섹션 범위 안의 탭/언어 선택자 처리
            tablists = []
            ...
            for idx, tablist in enumerate(tablists, start=1):
                ...
                # 2-1) 보이는 탭 먼저
                visible_labels = [_tab_label(t, f"Tab {idx}") for t in _get_tabs(tablist)]
                for lab in visible_labels:
                    if lab in clicked:
                        continue
                    res = _collect_for_label(tablist, lab)
                    if res:
                        code_items.extend(res); clicked.add(lab)

                # 2-2) 더보기(overflow) 항목들
                try:
                    more = _find_more_button(tablist)
                    if more:
